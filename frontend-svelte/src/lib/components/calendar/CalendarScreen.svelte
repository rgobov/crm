<script>
    ...
    function getWorkloadColor(count) {
        ...
        if (count <= 2) return '#dcfce7'; // green
        if (count <= 5) return '#fef9c3'; // yellow
        if (count <= 8) return '#ffedd5'; // orange
        return '#fee2e2'; // red
    }
...
<style>
    .calendar-container { padding: 16px; background: white; border-radius: 24px; box-shadow: var(--shadow); }
    .cal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
    .cal-header h3 { margin: 0; font-size: 18px; font-weight: 800; color: #0f172a; }
    .cal-header button { background: #f1f5f9; border: none; width: 40px; height: 40px; border-radius: 12px; font-size: 24px; cursor: pointer; color: var(--primary-color); }
    ...
    .days-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 6px; transition: opacity 0.2s; }
    .loading { opacity: 0.5; }

    .day-cell { aspect-ratio: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; border-radius: 14px; cursor: pointer; position: relative; }
    ...
    .workload-dot { position: absolute; bottom: 4px; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 800; color: #1e293b; }
